...
class formalEventsMediaViewController: UIViewController, UICollectionViewDataSource
{
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        eventName?.frame = CGRect(x: 43, y: name!.frame.maxY - 3, width: teamNameWidth, height: teamNameHeight)
        
        let collectionViewWidth = bounds.size.width * 0.87
        let collectionViewHeight = bounds.size.height * 0.76
        collectionView?.frame = CGRect(x: 26, y: eventName!.frame.maxY + 8, width: collectionViewWidth, height: collectionViewHeight)
    }
